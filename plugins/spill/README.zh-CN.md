...
- `dir`（默认 `<系统临时目录>/mcp-toolify/spill`）：落盘目录。启动时权限收紧为 `0700`，
  目录不属于本进程或收紧失败则拒绝启动；校验通过后以句柄形式持有（`os.OpenRoot`），
  启动之后把路径换成软链既改不了写入位置也读不出别处的文件。宿主也可以用
  `spill.SetDefaultDir(dir)`（须在 `Install` 之前）给一个运行期才知道的目录兜底
