// Command server 是 mcp-toolify 的组装示例：基座 + spill（大结果落盘）。
//
// 别的插件按同样方式装：import 后按「外 → 内」的顺序各调一次 Install（推荐顺序见 README
// 的「插件顺序」一节）。本仓库另带 audit（审计）；配额、审批一类由使用方以外置 module
// 提供——任何插件都必须能从仓库外装进来，否则基座就给自带插件开了特例。
//
